...
def index(request):
    content = paginate( filter_shadowbans(request,
...
def profile(request, profile_id):
    ...
    content = paginate( filter_shadowbans(request, 
                            Post.objects.filter(author=profile_id).order_by('-timestamp')),
                        request.GET.get('page'),
                        user=logged_in_user(request))
...
def following(request):
    ...
    content = paginate( filter_shadowbans(request,
                            Post.objects.filter(author__in=followed).order_by('-timestamp')),
                        request.GET.get('page'),
                        user=logged_in_user(request))
...
def edit_post(request, post_id):
    ...
    if not int(data['post_id']) == int(post_id):
        return JsonResponse({"warning": "Post ID in form action and input field named 'post-id'\
                            ...
                            modifying both values won't let you edit other users' posts. This \
                            security measure only exists to make it less probable for you to \
                            accidentally edit your other post if you were playing with dev tools."},
                            status=418)
...
def get_reactions(request, post_id):
    ...
    post = get_object_if_exists(Post, post_id)
    if not post:
        return JsonResponse(
...
def get_rcategories(request):
    ...
    if not request.user.is_authenticated:
        return JsonResponse({"unauthenticated": True}, status=200)
...
@login_required
def shadowban(request, target_id):
    require_method(request, 'POST')
    
    if not request.user.is_superuser:
        return JsonResponse(
            {   "Error": "Permission denied. Only superusers may modify shadowbanned status"
            },  status = 403)
    
    target = get_object_if_exists(User, target_id)

    if target.shadowbanned:
        target.shadowbanned = False
        action = 'shadowban removed'
    else:
